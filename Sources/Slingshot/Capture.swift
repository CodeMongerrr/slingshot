    ...
    func start() throws {
        ...
        if let builtIn = builtInInputDeviceID(), defaultInputDeviceID() != builtIn,
           let unit = input.audioUnit {
            ...
            log("🎙️ Listening on the system default microphone")
        }

        // Unpinned, the tap sees the node's client-side (output) format — the
        // stock path. Pinned, the node's cached client format can describe the
        // OLD device (an uncatchable installTap exception), so use the pinned
        // device's hardware format instead.
