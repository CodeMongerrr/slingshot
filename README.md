...
- **Clap to mute** (opt-in, off by default): one clap toggles the Mac's output mute — clap to silence it, clap again to bring the sound back. The notch shows "Muted" / "Sound on". Snaps and claps share one detector: only enabled features compete for a sound, and whichever class the classifier is more confident about wins, so one sound never triggers two actions. CoreAudio flips the default output device's mute; devices without a mute control fall back to the system volume setting. Sound features listen on the built-in microphone even when a Bluetooth speaker holds the system input — a far-field speaker mic hears neither snaps nor claps.
- **Auto-quit** (on by default): after 10 seconds with no hand in view, no snap or clap, and no transfer in flight, Slingshot quits itself — and three quick snaps (within 5 s) quit it on the spot. Toggle the idle timer from the menu bar. Once quit, reopen it from Finder: a snap cannot relaunch a closed app.
