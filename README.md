...
- **Clap actions**: by default a clap puts the camera to sleep, the mirror of snap-to-wake. Opt in to **clap-to-mute** and a clap instead toggles the Mac's output mute — clap to silence it, clap again to bring the sound back, with "Muted" / "Sound on" in the notch. CoreAudio flips the default output device's mute; devices without a mute control fall back to the system volume setting. Snaps and claps share one detector: only enabled features compete for a sound, and the higher-confidence class wins, so one sound never triggers two actions (`routeSoundWindow` in SlingshotCore, covered by tests). Sound features listen on the built-in microphone when a Bluetooth device holds the system input — a far-field speaker mic hears neither snaps nor claps.
- **Auto-quit** (opt-in, off by default): after 10 seconds with no hand in view, no snap or clap, and no transfer in flight, Slingshot quits itself — and, separately opt-in, three quick snaps (within 5 s) quit it on the spot. Both toggles live in the menu bar and the control panel. Once quit, reopen it from Finder: a snap cannot relaunch a closed app.
- **Control panel**: a small floating window (shown at launch, reopenable from the menu bar) with live lamps that glow the moment the recorder registers a snap or a clap, confidence meters, checkboxes for every sound feature, and a status readout that names missing permissions in plain text.
